...
static int getModemPath(io_iterator_t serialPortIterator, char *bsdPath, CFIndex maxPathSize)
{
    io_object_t		modemService;
    kern_return_t	kernResult = KERN_FAILURE;
    int8_t			modemsFound = 0;
    ...
    *bsdPath = '\0';
    ...
    while ((modemService = IOIteratorNext(serialPortIterator)) && (modemsFound < 6)) {
        ...
        if (bsdPathAsCFString) {
            ...
            result = //CFStringGetCString(<#CFStringRef theString#>, <#char *buffer#>, <#CFIndex bufferSize#>, <#CFStringEncoding encoding#>)
            CFStringGetCString((CFStringRef) bsdPathAsCFString,
                               bsdPath,
                               maxPathSize,
            ...
            if (result) {
                strcpy(myPorts[modemsFound],bsdPath);
                modemsFound ++;
                printf("Modem number %d found with BSD path: %s", modemsFound, bsdPath);
...
static void writeNull(int fileDescriptor) {
    write(fileDescriptor,"\0\0\0\0",4);
}
...
int getKeyForPort(int portIndex) {
    char theHitKey = ' ';
    Boolean stillWaiting = true;
    while (stillWaiting) {
        int fileDescriptor = openSerialPort(myPorts[portIndex]);
        printf("Hit 1-6 to set joint number, or Space to flash the TX light again...\n");
        
        writeNull(fileDescriptor);
...
bool readSerialSetupFromFile() {
    //bool _isLoaded = false;
    //using namespace std;
    int myCount = 0;
    std::string line;
    std::ifstream myFile ("serialPortSetup.txt");
    if (myFile.is_open())
    {
        myCount = 0;
        while ( getline (myFile,line) )
        {
            std::strcpy(myPorts[myCount],line.c_str());
            portMapping[myCount] = myCount;
            myCount++;
        }
        myFile.close();
        if (myCount == 6) {
...
bool checkPortsExist() {
    bool success = true;
    printf(" *----- Checking ports: \n");
    for (int i=0; i<6; i++) {
        //
        printf("     *------ Channel %d is port \"%s\". Opening... ", i, myPorts[portMapping[i]]);
        int fileDescriptor = openSerialPort(myPorts[portMapping[i]]);
        if (fileDescriptor == -1) {
            /// fail!
            success = false;
...
bool getSerialPorts() {
    //
    // needs to return true or false depending on success;
    // if true, myPorts and jointIndex will have been populated
    printf("Getting Serial ports...\n");
    bool success = false;
    if (readSerialSetupFromFile()) {
        printf(" *-- Loaded from file \n");
        success = checkPortsExist();
    }
    if (!success) {
        printf(" *-- Loading from file failed. Doing normal setup...\n");
        doSerialSetup();
        success = checkPortsExist();
    }
    return success;
...
void writePortsToSetupFile() {
    FILE * pFile;
    pFile = fopen ("serialPortSetup.txt","w");
    for (int i=0; i<6; i++) {
...
int doSerialSetup()
{
    ...
    //   if (readSerialSetupFromFile((char *) "serialSetup.txt")) {
    //           // check ports exist etc
    //   }
    ...
    int numOfPortsFound = getModemPath(serialPortIterator, bsdPath, sizeof(bsdPath));
    if (numOfPortsFound < 6) {
        printf("Could not find 6 ports.\n");
        assert(false);
    }
    ...
    for (int i=0;i<6;i++) {
        ...
        portMapping[getKeyForPort(i)-1] = i;
        
    }
    printf("\n\n");
    
    for (int i=0;i<6;i++) {
        printf("Dev %d is %s\n",i,myPorts[portMapping[i]]);
    }
    
    
    writePortsToSetupFile();
